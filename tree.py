...
class Tree:
    ...
    def buildTree(self, source):
        ...
        if type(source) == str and source != '':
            ...
            if self.tree.keys(): self.viable = 1
        else: return
    ...
    def TSV(self, table):
        ...
        if table == 'base':
            tsv = '\t'.join(['Node','Mutations','Leaves','Rho','Standard error','Age','Confidence interval'])
            for node in self.tree.values():
                tsv += '\r\n'
                if node.name in self.leaves:
                    tsv += '\t'.join([node.name, str(len(node.mutations)), '--','--','--','--','--'])
                elif node.name in self.nodes:
                    xnode = self.subtrees[node.name]
                    tsv += '\t'.join([str(i) for i in [node.name, len(node.mutations), len(set(xnode.keys()) & set (self.leaves)), \
                self.Rho(node.name, xnode), self.StErr(xnode), self.Age(node.name), '%s - %s' % self.ConfidenceInterval(node.name)]])
        elif table == 'f1':
            tsv = '\t'.join(['Node','Mutations','Type','Leaves','f1','Standard error'])+'\n'
            for node in self.tree.values():
                tsv += '\r\n'
                f1 = self.fN(node.name, 1)
                tsv += '\t'.join([str(i) for i in [node.name, len(node.mutations), node.isSource() if node.name in self.leaves else '--', f1[0], f1[1], f1[2]]]) 
        elif table == 'f2':
            tsv = '\t'.join(['Node','Mutations','Type','Leaves','f2','Standard error','f2+'])
            for node in self.tree.values():
                tsv += '\r\n'
                f2 = self.fN(node.name, 2)                
                tsv += '\t'.join([str(i) for i in [len(node.mutations), node.isSource() if node.name in self.leaves else '--', f2[0], f2[1], f2[2], self.f2plus(node.name)]])
        elif table == 'pf1':
            probs = self.probabilities[1]
            tsv = '\t'+'\t'.join([str(i) for i in probs.values()[0]])
            for node in probs.keys()[1:]:
                tsv += '\r\n'
                tsv += node + '\t' + '\t'.join([str(i) for i in probs[node]])
        elif table == 'pf2':
            probs = self.probabilities[2]
            tsv = '\t'+'\t'.join([str(i) for i in probs.values()[0]])
            for node in probs.keys()[1:]:
                tsv += '\r\n'
                tsv += node + '\t' + '\t'.join([str(i) for i in probs[node]])
    ...
    def Rho(self, root, sub, f = False):
        ...
        if not f and self.tree[root].extra['Rho'] != '--':
            return round(self.tree[root].extra['Rho'], 3)
        else:
            mutCount = 0
    ...
    def StErr(self, sub, f = False):
        ...
        if not f and sub.values()[0].extra['SE'] != '--':
            return round(sub.values()[0].extra['SE'], 3)
        tSum = 0.0
    ...
    def migrationProbs(self, migrations, mutationRate, f, effective):
        ...
        probabilities = odict()
        probabilities['%s %s' % (mutationRate, effective)] = migrations
        ...
        self.probabilities[f] = probabilities
